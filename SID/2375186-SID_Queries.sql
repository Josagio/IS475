...
FROM STUDENT
JOIN JOINS ON STUDENT.NSHE_ID = JOINS.NSHE_ID
JOIN CLUB ON CLUB.Club_ID = JOINS.Club_ID
GROUP BY CLUB.Club_Name;


SELECT COUNT(STUDENT.NSHE_ID) AS Enrolled_Students, CLUB.Club_Name
FROM STUDENT
JOIN JOINS ON STUDENT.NSHE_ID = JOINS.NSHE_ID
JOIN CLUB ON CLUB.Club_ID = JOINS.Club_ID
WHERE STUDENT.Enrollment = 'P'
GROUP BY CLUB.Club_Name;
...
WHERE STUDENT.Enrollment = 'P'
GROUP BY CLUB.Club_Name
HAVING COUNT(STUDENT.NSHE_ID) >= 5;