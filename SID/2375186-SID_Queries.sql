...
SELECT Club_Name, Budget, Purpose FROM CLUB WHERE Budget > 5000;
...
#The number of students in each club:
SELECT CLUB_ID, COUNT(NSHE_ID) AS StudentCount
FROM JOINS
GROUP BY CLUB_ID;
...
SELECT CLUB.Club_Name AS 'Ineligible Club Name', COUNT(DISTINCT JOINS.NSHE_ID) AS '# of Student Members'
FROM CLUB
